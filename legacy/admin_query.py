...
def admin_read_companies(cur) -> List[Dict[str, Any]]:
    """
    Get all production companies ordered by name.
    Returns list of company dicts.
    """
    cur.execute("SELECT company_id, name FROM production_companies ORDER BY name")
...
# -------------------------
# Movie Production Companies CRUD (junction)
# -------------------------
...
cur = conn.cursor()
test = admin_get_movie_cast(cur, 15602, 1)
print(test)
cur.close()