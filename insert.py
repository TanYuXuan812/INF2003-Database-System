...
def parse_json_field(field_str):
    """Parse string representation of JSON to actual JSON"""
    try:
...
with open('updated_data/movies_metadata.csv', 'r', encoding='utf-8', errors='replace') as f:
    reader = csv.reader(f)
    ...
    for row in reader:
        ...
        try:
            ...
            movie_id = int(row[0])
            adult = row[1].upper() == 'TRUE' if row[1] else False
            genres = parse_json_field(row[2]) if len(row) > 2 else []
            ...
            production_companies = parse_json_field(
                row[8]) if len(row) > 8 else []
            released_date = parse_date(row[9]) if len(row) > 9 else None
            # revenue = row[10] (ignored)
            runtime = int(row[11]) if (len(row) > 11 and row[11]) else None
...
keyword_count = 0
keyword_errors = 0
with open('original_data/keywords.csv', 'r', encoding='utf-8', errors='replace') as f:
    reader = csv.reader(f)
    header = next(reader, None)
    for row in reader:
        cur = get_cursor()
...
credit_count = 0
credit_errors = 0
with open('original_data/credits.csv', 'r', encoding='utf-8', errors='replace') as f:
    reader = csv.reader(f)
    header = next(reader, None)
    for row in reader:
        cur = get_cursor()
...
def create_synthetic_users_from_ratings():
    ...
    with open('updated_data/ratings_small.csv', 'r', encoding='utf-8', errors='replace') as f:
        reader = csv.reader(f)
        header = next(reader, None)
        for row in reader:
            try:
